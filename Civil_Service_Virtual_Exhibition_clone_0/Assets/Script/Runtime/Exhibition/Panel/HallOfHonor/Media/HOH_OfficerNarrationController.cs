...
public class HOH_OfficerNarrationController : MonoBehaviour, IMediaControllable
{
    ...
    public bool HasTarget => _currentPerson != null;
    ...
    public void BindPerson(HOH_PersonDto person)
    {
        if (ReferenceEquals(_currentPerson, person))
            return;
    ...
    public void ClearTarget(bool restoreBgm = true)
    {
        ResetSession(restoreBgm);
        _currentPerson = null;
    }

    public void ToggleBoundPerson()
    {
    ...
    public void PlayBoundPerson()
    {
        string url = BuildOfficerTtsUrl(_currentPerson);
        Debug.Log($"[HOH_OfficerNarrationController] Narrator URL = {url}");
    ...
    private IEnumerator DownloadAndPlay(string url)
    {
        ...
        Debug.Log(
            $"[HOH_OfficerNarrationController] TTS response | code={finishedRequest.responseCode} | " +
